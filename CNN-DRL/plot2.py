...
def backtest_plot(
    ...
    baseline_df = get_baseline(
        ticker=baseline_ticker, start_date=baseline_start, end_date=baseline_end,
        data_source=data_source, time_interval=time_interval
    )
...
def get_baseline(ticker, start_date, end_date, data_source='yahoofinance', time_interval='1D'):
    dp = DataProcessor(data_source=data_source,
                    start_date=start_date,
                    end_date=end_date,
                    time_interval=time_interval)
    dp.download_data(ticker_list=[ticker])
    return dp.dataframe
